...
mainAgeing = function(file = NULL                                    ,
  ...
  # log weight
  new.data$weight[new.data$weight > 0 &
                    !is.na(new.data$weight)] = log(new.data$weight[new.data$weight > 0 &
                                                                     !is.na(new.data$weight)])
  # log response
  new.data$data_point[new.data$data_point > 0 &
                        !is.na(new.data$data_point)] = log(new.data$data_point[new.data$data_point > 0 &
                                                                                 !is.na(new.data$data_point)])
                ...
                if (BatchProducer && nColonies > 0) {
                  ...
                  MultiCoreRes = foreach::foreach (
                    ...
                    for (sim.index in 1:ifelse(simulation, Simulation.iteration, 1)) {
                      ...
                      # Summary statistics
                      n3.5_summary = SummaryStatisticsOriginal(
                        x      = n3.5,
                        depVar = depVar,
                      ...
                      if (storeRawData) {
                        ...
                        RawoutputFile = RawoutputFile0 = file.exists0(paste(outpfile2,
                                                                            ...
                                                                            sep = '_'),
                                                                      overwrite  = OverwriteExistingFiles)
                        ReadMeFile  = file.exists0(file.path(
                          dirname(RawoutputFile),
                          'ReadMe.txt'
                        ))
                        message0(
                      ...
                      if (!is.null(n3.5.2) &&
                        ...
                        if (!is.null(WhiteListMethods) &&
                          ...
                          message0('Black list applied. Method = ',
                                   method)
                          return(FALSE)
                        }
