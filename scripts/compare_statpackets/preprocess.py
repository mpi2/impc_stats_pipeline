...
assert sys.version_info >= (3, 7), (
    "The script requires Python 3.7+ to work, because it depends on stable dictionary "
    "sort order."
)
...
def recursively_normalise(list_dict_or_other):
    ...
    if type(list_dict_or_other) is dict:
        return {
            k: recursively_normalise(list_dict_or_other[k])
            for k in sorted(list_dict_or_other.keys())
        }
    else:
...
def normalise_and_serialise(list_dict_or_other):
    ...
    list_dict_or_other["Details"]["DRversion"] = "xx.x"
    list_dict_or_other["Details"]["Input file"] = re.sub(r"(/tmp/)[^/]+", r"\1id", list_dict_or_other["Details"]["Input file"])
    list_dict_or_other["Details"]["Input file"] = re.sub(r"_(\d+\.\d+)_", r"_seed_", list_dict_or_other["Details"]["Input file"])
    list_dict_or_other["Details"]["statpacket_raw_id"] = re.sub(r"(DRversion:\[)\d+\.\d+", r"\1xx.x", list_dict_or_other["Details"]["statpacket_raw_id"])
    list_dict_or_other["Details"]["statpacket_universal_id"] = "id"
