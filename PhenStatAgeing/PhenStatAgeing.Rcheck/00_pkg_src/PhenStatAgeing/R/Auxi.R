...
checkModelTermsInData = function(formula,
	...
	if (any(!In)) {
		message0('Some parts of the model does not included in the data. See: ',
						 pasteComma(vars[!In]))
		ft  = vars [!In]
...
TypicalModel = function(depVariable,
	...
	if ((!is.null(fbm)         &&
			 min(fbm$min.freq) < 1) ||
			length(formulaTerms(fixed)) != nrow(fbm))
...
dim0 <- function( ... ){
	args <- list(...)
	sapply( args , function(x) { if( is.null( dim(x) ) )
		return( length(x) )
		dim(x) } )
}
...
ctest = function(x,
	...
	if (
		...
		message = c(message, 'No variation in al least two levels of one/more category(s)')
		return(list(
			result = NULL,
			effectSize = NULL,
			note    = message,
...
AllTables = function(dframe        = NULL,
	...
	for (i in unique(pmax(1, 1:(lcat - 1)))) {
		cb = combn(cat, i)
		for (j in 1:ncol(cb)) {
			message0('\tspliting on ',pasteComma(cb[, j]))
			out = split(dframe,
...
RRNewObjectAndFormula = function(object, RRprop, formula, labels = NULL) {
...
RRCut = function(object                    ,
	...
	newdepVariable                  = pasteUnderscore(depVariable, 'discretised')
...
RRextra = function(object,
	...
	return(list(overall = tbl, tblLow = tblLow, tblHigh= tblHigh))
}
...
SummaryStats = function(x,
	...
	summaryT   = as.list(tapply(x[, depVar], INDEX = lvls, function(xx) {
		if (isNumeric) {
			...
			c = ifelse(length(na.omit(xx)) > 0, length(na.omit(xx)), 0)
			r = list(count = c,mean=NULL,sd=NULL)
		}
		return(r)
