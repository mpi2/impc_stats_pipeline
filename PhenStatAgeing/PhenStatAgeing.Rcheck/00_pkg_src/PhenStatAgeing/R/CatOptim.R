									 ...
									 method        = NULL)
{
	if (sum(method != c('FE', 'RR')) == 0) {
		message0 ('Improper method (', method, ') for the type of data!')
		return(NULL)
	}
	
	sta.time    = Sys.time()
	message0('Framework: ', method)
	...
	####
	lComplete   = NULL
	for (indx in 1:pmax(1, length(allTerms) - 1)) {
		...
		for (j in 1:length(vars)) {
			message0('Testing for the main effect ', pasteComma(vars[j]))
	...
	message0('FE framework executed in ', round(difftime(Sys.time() , sta.time, units = 'sec')), ' seconds')
	OutR = list(
	...
	class(unclass(OutR)) = 'PhenStatAgeingFE'
	return(OutR)
}
