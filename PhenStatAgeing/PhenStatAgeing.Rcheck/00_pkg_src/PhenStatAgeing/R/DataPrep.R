PhenListAgeing = function(object,
	...
	if (length(LL) != 2 && debug) {
		message0('There must be two levels in the LifeStage. Levels: ',
						 pasteComma(LL))
		#return(NULL)
	}
	if (length(LS) != 2 && debug) {
		message0('There must be two levels in Sex. Levels: ', pasteComma(LS))
		#return(NULL)
	}
	...
	if (length(LG) < 2 && debug) {
		message0('Genotype must have two levels. Levels: ', pasteComma(LG))
		return(NULL)
	}
